...
until mysql -h172.28.3.4 -uroot -proot -N -e "SELECT COUNT(*) FROM performance_schema.replication_connection_status WHERE SERVICE_STATE='ON'" 2>/dev/null | grep -q 2; do
    sleep 2
done

# Create tables on each primary
mysql -h172.28.3.2 -uroot -proot source_a -e "
...
"
mysql -h172.28.3.3 -uroot -proot source_b -e "
    CREATE TABLE IF NOT EXISTS load_test (
        id BIGINT AUTO_INCREMENT PRIMARY KEY,
        data VARCHAR(255),
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
