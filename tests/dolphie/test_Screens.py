...
async def test_main_screen_keeps_help_in_topbar_without_footer():
...
async def test_daemon_initializes_without_graph_dashboard():
...
async def test_graph_dashboard_mounts_without_dynamic_tab_graph_attributes():
...
async def test_event_log_moves_inline_help_to_contextual_footer():
...
async def test_thread_screen_exposes_copy_actions_instead_of_buttons():
...
async def test_tab_setup_defaults_unknown_ssl_configuration_to_required():
