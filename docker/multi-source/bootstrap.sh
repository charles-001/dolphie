...
for host in 172.28.2.2 172.28.2.3; do
...
mysql -h172.28.2.2 -uroot -proot -e "CREATE DATABASE IF NOT EXISTS source_a;"
mysql -h172.28.2.3 -uroot -proot -e "CREATE DATABASE IF NOT EXISTS source_b;"

# Wait for the replica to be ready
until mysql -h172.28.2.4 -uroot -proot -e "SELECT 1" 2>/dev/null; do
    sleep 1
done

# Reset replica's binary logs
mysql -h172.28.2.4 -uroot -proot -e "RESET BINARY LOGS AND GTIDS;" 2>/dev/null

# Configure multi-source replication channels on the replica
mysql -h172.28.2.4 -uroot -proot <<-EOSQL
    CHANGE REPLICATION SOURCE TO
        SOURCE_HOST='172.28.2.2',
        SOURCE_USER='repl',
